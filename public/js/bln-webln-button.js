  ...
  (function () {
    ...
    function checkPaymentStatus(invoice, callback) {
      ...
      return function () {
        ...
        fetch(wp_rest_base_url + "/invoices/verify", {
          method: "POST",
    ...
    function showQRCode(invoice, options) {
      ...
      qrCodeContainer.className = "wp-lnp-qrcode";

      var qrCodeLink = document.createElement("a");
      qrCodeLink.href = "lightning:" + invoice.payment_request;
      qrCodeLink.target = "_blank";

      var qrCodeCanvas = document.createElement("canvas");
      qrCodeCanvas.setAttribute("id", "ln-qr-code");
      qrCodeLink.appendChild(qrCodeCanvas);

      var qrCodeText = document.createElement("p");
      qrCodeText.innerHTML = invoice.payment_request.substr(0, 36) + "...";

      var copyButton = document.createElement("span");
      copyButton.className = "wp-lnp-copy";
      copyButton.innerHTML = LN_Paywall_Copy;
      ...
      options.target.appendChild(qrCodeContainer);
    ...
    function requestPayment(params, options) {
      return fetch(wp_rest_base_url + "/invoices", {
        method: "POST",
        credentials: "same-origin",
        cache: "no-cache",
        ...
        .then(function (invoice) {
          if (!invoice || !invoice.payment_request) {
            ...
            throw new Error(
              "Failed to generate lightning invoice " + JSON.stringify(invoice)
            );
          }
    ...
    function initWeblnButtons() {
      ...
      buttons.forEach(function (button) {
        button.addEventListener("click", function (e) {
          ...
          wrapper.innerHTML =
            '<div class="wp-lnp-webln-button-spinner">' +
            LN_Paywall_Spinner +
            "</div>";

          requestPayment(paymentRequestArgs, { target: wrapper })
            .then(function (invoice) {
              wrapper.innerHTML =
                '<div class="wp-lnp-webln-button-thanks">' +
                thankyouMessage +
                "</div>";
            })
            .catch(function (e) {

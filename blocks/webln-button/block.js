(function (blocks, i18n, element, components, _, blockEditor) {
  ...
  blocks.registerBlockType("alby/webln-button", {
    attributes: {
      amount: {
        type: "number",
      },
    ...
    edit: function (props) {
      const { amount, currency, button_text, success_message } =
        props.attributes;

      return el("div", useBlockProps({ className: props.className }), [
        el("h4", {}, "Donation Button"),
        el(TextControl, {
          label: __(
            "Amount (in the smallest unit (sats/cents e.g. for $4.90 it is 490))",
            "alby"
          ),
          onChange: (v) => {
            if (v !== "") {
